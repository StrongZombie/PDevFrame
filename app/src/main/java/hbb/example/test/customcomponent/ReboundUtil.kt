...
class ReboundUtil {
    ...
    fun addRebound(activity: Activity, type:Int,scrollView: View?=null):ReboundUtil{
        when(type){
            ...
            NON_SLIING -> addNormalRebound(activity)
        }
        return this
    }
    ...
    fun setHeadBoundView(headertView: View):ReboundUtil{
        ...
        if (rootPostFinish){
            reboundLayout?.let {
                if (headertView.parent==null){

                    reboundLayout?.addView(headertView,0)

                    gListener = ViewTreeObserver.OnGlobalLayoutListener{
                        if (headertView?.height>0){
                            reboundLayout?.setHeaderHeight(headertView?.height)
        ...
        if (rootPostFinish){
            reboundLayout?.let{
                if (bottomView.parent==null){
                    ...
                    reboundLayout?.addView(bottomView,reboundLayout!!.childCount)

                    gListener = ViewTreeObserver.OnGlobalLayoutListener{
                        if (bottomView?.height>0){
                            reboundLayout?.setBottomHeight(bottomView?.height)
